...
void Machine::OneInstruction(Instruction *instr) {
	...
	// LB: Added to handle the >> operator correctly in the SRL*
	// instructions
	...
	case OP_BEQ:
		if (registers[instr->rs] == registers[instr->rt])
			pcAfter =
			    registers[NextPCReg] + IndexToAddr(instr->extra);
		break;
	...
	case OP_SB:
		if (!machine->WriteMem(
			(unsigned)(registers[instr->rs] + instr->extra), 1,
			registers[instr->rt]))
			return;
		break;
	...
	case OP_SH:
		if (!machine->WriteMem(
			(unsigned)(registers[instr->rs] + instr->extra), 2,
			registers[instr->rt]))
			return;
		break;
	...
	case OP_SW:
		if (!machine->WriteMem(
			(unsigned)(registers[instr->rs] + instr->extra), 4,
			registers[instr->rt]))
			return;
		break;
	...
	case OP_SWR:
		...
		case 0:
			value =
			    (value & 0xffffff) | (registers[instr->rt] << 24);
			break;
		case 1:
