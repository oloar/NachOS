...
char *argv[];
{
	...
	for (;;) {
		...
		if (instr != 0) /* eliminate no-ops */
		{
			...
			case I_BCOND: {
				switch (
				    rt(instr)) /* this field encodes the op */
				{
				case I_BLTZ:
					if (Reg[rs(instr)] < 0)
						npc = xpc + 4 +
			...
			case I_LB:
				Reg[rt(instr)] =
				    cfetch(Reg[rs(instr)] + immed(instr));
				break;
			case I_LH:
				Reg[rt(instr)] =
				    sfetch(Reg[rs(instr)] + immed(instr));
				break;
			case I_LWL:
			...
			case I_LW:
				Reg[rt(instr)] =
				    fetch(Reg[rs(instr)] + immed(instr));
				break;
			case I_LBU:
				Reg[rt(instr)] =
				    ucfetch(Reg[rs(instr)] + immed(instr));
				break;
			case I_LHU:
				Reg[rt(instr)] =
				    usfetch(Reg[rs(instr)] + immed(instr));
				break;
			case I_LWR:
				i = Reg[rs(instr)] + immed(instr);
				Reg[rt(instr)] &= (-1 << 8 * (i & 0x03));
			...
			case I_SB:
				cstore(Reg[rs(instr)] + immed(instr),
				       Reg[rt(instr)]);
				break;
			case I_SH:
				sstore(Reg[rs(instr)] + immed(instr),
				       Reg[rt(instr)]);
				break;
			case I_SWL:
				...
				break;
			case I_SW:
				store(Reg[rs(instr)] + immed(instr),
				      Reg[rt(instr)]);
				break;
	...
	for (h = 0; h <= NH; ++h)
		if (htotal[h] > 0) {
			...
			printf("oflo %d:\t%d/%d\t%5.2f%%\n", h, hoflo[h],
			       htotal[h], (double)100 * hoflo[h] / htotal[h]);
		}
}
