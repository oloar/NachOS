...
int main(int argc, char **argv) {
	...
	for (i = 0; i < numsections; i++) {
		printf("\t\"%.8s\", filepos 0x%lx, mempos 0x%lx, size 0x%lx, "
		       "flags 0x%lx\n",
		...
		if (sections[i].s_size == 0) {
			...
			if (noffH.initData.size != 0) {
				fprintf(stderr,
					"Can't handle both data and rdata\n");
				unlink(noffFileName);
				exit(1);
			}
			...
			/* need to check if we have both .bss and .sbss -- make
			 * sure they are contiguous
			 */
			if (noffH.uninitData.size != 0) {
				if (sections[i].s_paddr ==
					...
					fprintf(
					    stderr,
					    "Can't handle both bss and sbss\n");
					unlink(noffFileName);
					exit(1);
				}
			...
			for (car = buffer; car < buffer + sections[i].s_size;
			     car += 8) {
				printf("\t\t");
				car_lim = car + 8;
